...
for entry in \
	...
	"fc_base,img_mnist_0.048508.txt,not verified" \
	"fc_base,img_mnist_0.048839.txt,verified" \
	...
	"conv_base,img_mnist_0.075798.txt,not verified" \
	"conv_base,img_mnist_0.055929.txt,verified" \
...
do
	...
	output=$({ time (timeout 60s python code/verifier.py --net ${net} --spec test_cases/${net}/${spec} || echo not verified); } 2>&1)
